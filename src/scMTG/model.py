...
class Generator(tf.keras.Model):
    """Generator network.
    """
    def __init__(self, input_dim, z_dim, output_dim, model_name, nb_layers=2, nb_units=256, concat_every_fcl=True, batchnorm=False):  
    ...
    def call(self, inputs, training=True):
        ...
        fc_layer, norm_layer = self.all_layers[-1]
        with tf.name_scope("%s_g_layer_ouput"%self.model_name):
            output = fc_layer(x)
            # No activation func at last layer
            #x = tf.keras.layers.LeakyReLU(alpha=0.2)(x)
        return output
